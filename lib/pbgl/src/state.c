...
void pbgl_state_init(void) {
	...
	pbgl.colormask.value =
		NV097_SET_COLOR_MASK_RED_WRITE_ENABLE | NV097_SET_COLOR_MASK_GREEN_WRITE_ENABLE | NV097_SET_COLOR_MASK_BLUE_WRITE_ENABLE | NV097_SET_COLOR_MASK_ALPHA_WRITE_ENABLE;
	pbgl.colormask.dirty = GL_TRUE;

	for(int i = 1; i < LIGHT_COUNT; ++i) {
		pbgl.light[i].ambient = (vec4f){
			{0.f, 0.f, 0.f, 1.f}
		};
		pbgl.light[i].diffuse = (vec4f){
			{0.f, 0.f, 0.f, 1.f}
		};
		pbgl.light[i].specular = (vec4f){
			{0.f, 0.f, 0.f, 1.f}
		};
		pbgl.light[i].pos = (vec4f){
			{0.f, 0.f, 1.f, 0.f}
		};
	...
	pbgl.light[0].ambient = (vec4f){
		{1.f, 1.f, 1.f, 1.f}
	};
	pbgl.light[0].diffuse = (vec4f){
		{1.f, 1.f, 1.f, 1.f}
	};
	pbgl.light[0].specular = (vec4f){
		{1.f, 1.f, 1.f, 1.f}
	};
	pbgl.light[0].pos = (vec4f){
		{0.f, 0.f, 1.f, 0.f}
	};
	...
	pbgl.varray[VARR_COLOR1].value = (vec4f){
		{1.f, 1.f, 1.f, 1.f}
	};
	pbgl.varray[VARR_NORMAL].value = (vec4f){
		{0.f, 0.f, 1.f, 0.f}
	};
...
static inline void set_feature(GLenum feature, GLboolean value) {
	...
	case GL_LIGHTING:
		...
		if(value) {
			pbgl.mtx_any_dirty = pbgl.mtx[MTX_MODELVIEW].dirty = GL_TRUE;
		}
		break;
	...
	case GL_LINE_SMOOTH:
		...
		break;
	default:
		pbgl_set_error_arg(GL_INVALID_ENUM, feature);
		break;
	}
}
...
GL_API void glColorMask(GLboolean r, GLboolean g, GLboolean b, GLboolean a) {
	...
	if(r) {
		pbgl.colormask.value |= NV097_SET_COLOR_MASK_RED_WRITE_ENABLE;
	}
	if(g) {
		pbgl.colormask.value |= NV097_SET_COLOR_MASK_GREEN_WRITE_ENABLE;
	}
	if(b) {
		pbgl.colormask.value |= NV097_SET_COLOR_MASK_BLUE_WRITE_ENABLE;
	}
	if(a) {
		pbgl.colormask.value |= NV097_SET_COLOR_MASK_ALPHA_WRITE_ENABLE;
	}
