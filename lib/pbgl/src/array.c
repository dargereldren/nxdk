...
static inline GLuint *array_push_format(GLuint *p, GLuint index, GLuint format, GLuint size, GLuint stride) {
	return push_command_parameter(p, NV097_SET_VERTEX_DATA_ARRAY_FORMAT + index * 4, PBGL_MASK(NV097_SET_VERTEX_DATA_ARRAY_FORMAT_TYPE, format) | PBGL_MASK(NV097_SET_VERTEX_DATA_ARRAY_FORMAT_SIZE, size) | PBGL_MASK(NV097_SET_VERTEX_DATA_ARRAY_FORMAT_STRIDE, stride));
}
...
static inline GLuint *push_draw_arrays(GLuint *p, GLuint start, GLuint count) {
	return push_command_parameter(p, 0x40000000 | NV097_DRAW_ARRAYS, PBGL_MASK(NV097_DRAW_ARRAYS_COUNT, (count - 1)) | PBGL_MASK(NV097_DRAW_ARRAYS_START_INDEX, start));
}
