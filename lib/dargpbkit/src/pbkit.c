...
static void pb_load_gr_ctx(int ctx_id) {
	...
	if(ctx_id == NONE) {
		...
		if(pb_3DGrCtxInst[ctx_id]) {
			...
			(void)VIDEOREG(NV_PGRAPH_DEBUG_0);
...
static void __stdcall DPC(PKDPC Dpc, PVOID DeferredContext, PVOID SystemArgument1, PVOID SystemArgument2) {
	...
	do {
		...
		if((VIDEOREG8(NV_PVIDEO_INTR) & NV_PVIDEO_INTR_BUFFER_0_PENDING) ||
		   (status & NV_PMC_INTR_0_PVIDEO_PENDING)) {
...
void pb_assign_tile(int tile_index, DWORD tile_addr, DWORD tile_size, DWORD tile_pitch, DWORD tile_z_start_tag, DWORD tile_z_offset, DWORD tile_flags) {
	DWORD old_dma_push;
...
static inline void pb_create_gr_instance(int ChannelID, int Class, DWORD instance, DWORD flags, DWORD flags3D, struct s_CtxDma *pGrObject) {
...
void pb_create_gr_ctx(int ChannelID, int Class, struct s_CtxDma *pGrObject) {
	DWORD flags;
	DWORD flags3D;
	int size;
	DWORD Inst;
	...
	if(pb_trace_mode) // do we want to wait until block data has been sent (for debugging GPU errors)?
	{
		...
		while(pb_busy()) {
			...
			if(TimeStamp2 - TimeStamp1 > TICKSTIMEOUT) {
				debugPrint("pb_end: Busy for too long (%lu) (%08x)\n", ((DWORD)(pb_Put) - (DWORD)(pb_Head)), VIDEOREG(NV_PFIFO_CACHE1_DMA_GET));
				break;
			}
		}
	}
